...
##################################################################################
# CREATE AWS RESOURCES USING AWS CLI (WITHOUT TERRAFORM)
##################################################################################

# Set AWS profile to use default
...
vpc_id=$(aws ec2 describe-vpcs \
  --region 'eu-west-3' \
  --filters Name="tag:Name",Values="globo-primary" \
  --query 'Vpcs[0].VpcId' --output text)

# Get the third AZ in the region
az=$(aws ec2 describe-availability-zones \
  --region 'eu-west-3' \
  --query 'AvailabilityZones[2].ZoneId' \
  --output text)
...
priv_rt=$(aws ec2 create-route-table --region 'eu-west-3' --vpc-id $vpc_id)
...
aws ec2 associate-route-table --region 'eu-west-3' --route-table-id $priv_rt_id --subnet-id $priv_subnet_id

# Get the public route table
pub_rt_id=$(aws ec2 describe-route-tables \
  --region 'eu-west-3' \
  --filters Name="vpc-id",Values=$vpc_id Name="tag:Name",Values="globo-primary-public" \
...
aws ec2 associate-route-table --region 'eu-west-3' --route-table-id $pub_rt_id --subnet-id $pub_subnet_id
